...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">ویرایش پرسش</h3>
        </div>
        <!-- /.card-header -->
        <!-- form start -->
        <form action="{{ route('faqs.update',['help'=>$faq->id]) }}" method="post">
            @csrf
            {{ method_field('PATCH') }}
            <div class="card-body">
                <div class="form-row">
                    <div class="form-group col-6">
                        <label for="title">عنوان پرسش</label>
                        <input type="text" class="form-control" id="title" name="title" value="{{ $faq->title ?? old('title') }}">
                    </div>
                    <div class="form-group col-6">
                        <label for="location">مکان نمایش</label>
                        <select class="form-control select2 w-100" id="location" name="location">
                            <option value="freelancer" {{ $faq->location == 'freelancer' ? 'selected' : '' }}>پرسش های مجریان</option>
                            <option value="employer" {{ $faq->location == 'employer' ? 'selected' : '' }}>پرسش های خریداران</option>
                            <option value="financial" {{ $faq->location == 'financial' ? 'selected' : '' }}>موارد مالی</option>
                            <option value="other" {{ $faq->location == 'other' ? 'selected' : '' }}>متفرقه</option>
                        </select>
                    </div>
                    <div class="form-group col-12">
                        <label for="content">توضیحات</label>
                        <textarea type="text" name="content" class="form-control editordata" id="content">{!! $faq->content ?? old('content') !!}</textarea>
                    </div>
                </div>
            </div>
            <!-- /.card-body -->
            <div class="card-footer">
                ...
                <a href="{{ route('faqs.index') }}" class="btn btn-default float-left">بازگشت</a>
            </div>
            <!-- /.card-footer -->
        </form>
    </div>
